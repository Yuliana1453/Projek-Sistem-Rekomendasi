# Laporan Proyek Rekomendasi Buku (Collaborative Filtering) - Yuliana
...
- Books.csv
  Berisi metadata buku seperti ISBN, judul, penulis, tahun terbit, penerbit, dan URL sampul buku. → Total: 271.379 buku
...
3. Variabel dalam Dataset Gabungan ratings
...
4. Kondisi Awal Data Gabungan
  1. Sebagian besar rating = 0 (implisit) → Hanya rating eksplisit (>0) yang dipertahankan untuk modeling.
  2. Distribusi rating tidak merata → Sebagian besar pengguna hanya memberi sedikit rating → Filtering pengguna aktif yang memberi ≥5 rating.
  3. Filtering buku populer → Hanya menyertakan buku yang mendapat ≥5 rating untuk menjamin relevansi model.
  4. Merge antar file, file Ratings.csv digabung dengan Books.csv untuk mendapatkan metadata buku → diperoleh data gabungan yang terdiri dari 137.214 interaksi user-book-rating.
  5. Kondisi awal data gabungan bersih, yaitu tidak memiliki missing value dan duplikat data.
...
1. Cek missing value dan duplikat data (tidak ada).
...
1. Membagi Data untuk Training dan Validasi
  ...
  Langkah-langkahnya adalah:
    1. Mengacak data
    2. Menyiapkan input dan label
    3. Membagi data menjadi train dan validasi
...
2. Membangun Arsitektur Model-based Collaborative Filtering
